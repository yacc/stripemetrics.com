class CancellationMetric < Metric
  ...
  def refresh!
    return if self.user.cancellation_trend.monthly.empty?
    self.this_month = self.user.cancellation_trend.monthly.last[1]
    self.reset!
    historical_data_size = self.user.cancellation_trend.monthly.size

    if historical_data_size >= 2
      self.last_month = self.user.cancellation_trend.monthly[-2][1]
      if historical_data_size >= 6
        last_six_month_data = self.user.cancellation_trend.monthly[-6..-1]
